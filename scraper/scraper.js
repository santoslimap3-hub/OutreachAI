...
async function extractThreadedComments(page, postUrl, targetName) {
    ...
    // Scroll again after expanding replies and see-more
    await page.evaluate(function() { window.scrollTo(0, document.body.scrollHeight); });
    await sleep(300);
    ...
    try {
        ...
        for (var batch = 0; batch < allPosts.length; batch += CONFIG.parallel) {
            ...
            results.forEach(function(r) {
                ...
                if (r.scottInvolved) {
                    dataset.metadata.postsWithTargetResponses++;
                    batchScott++;
                }
