...
def build_system_prompt(entry)
  tone = entry["tone"]
  intent = entry["intent"]
  sales_stage = entry["sales_stage"]

  has_tags = (tone.is_a?(Array) && !tone.empty?) ||
             (intent.is_a?(String) && !intent.empty?) ||
             (sales_stage.is_a?(String) && !sales_stage.empty?)

  return BASE_SYSTEM_PROMPT unless has_tags

  parts = [BASE_SYSTEM_PROMPT, "\nFor this reply:"]
  parts << "- Tone: #{tone.join(', ')}" if tone.is_a?(Array) && !tone.empty?
  parts << "- Intent: #{intent}" if intent.is_a?(String) && !intent.empty?
  parts << "- Sales stage: #{sales_stage}" if sales_stage.is_a?(String) && !sales_stage.empty?
  parts.join("\n")
end
...
def clean_body(body, category, title)
  ...
  # Remove category prefix (e.g. "Money 💰", "General 💭", "Mindset 🧠")
  # Pattern: category text appears at start of body, possibly with emoji
  if category && text.start_with?(category)
    text = text[category.length..].to_s
  end

  # Remove title duplication at start of body
  # The title often appears right after category removal
  if title && text.start_with?(title)
    text = text[title.length..].to_s
  end

  # Remove trailing numbers (engagement counts like "25", "12", "34")
...
def clean_comment(comment)
  ...
  # Remove trailing timestamps like "7:34", "4:57"
...
# Read source data
input_path = File.expand_path('../data/posts_with_scott_reply.json', __dir__)
...
File.open(output_path, 'w') do |f|
  data.each do |entry|
    post = entry["original_post"]
    comment = clean_comment(entry["comment"])

    # Skip low-quality one-liner replies
    ...
    # Build user message: what Scott sees before replying
    user_content = "Post by #{author}"
    user_content += " in [#{category}]" if category && !category.empty?
    user_content += ":\n\n"
    user_content += "**#{title}**\n\n#{body}"
...
puts "Done! Wrote #{kept} examples to #{output_path}"
puts "Skipped #{skipped} low-quality entries (comment < #{MIN_COMMENT_LENGTH} chars)"
