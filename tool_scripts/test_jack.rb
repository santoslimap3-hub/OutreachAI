...
# Read API key from rag/.env
env_file = File.join(File.dirname(__FILE__), '..', 'rag', '.env')
api_key = File.read(env_file).match(/ANTHROPIC_API_KEY=(.*)/)[1].strip
...
request.body = {
  ...
  messages: [
    { role: 'user', content: "Someone posted: #{post}. Respond as Scott." }
  ]
}.to_json
